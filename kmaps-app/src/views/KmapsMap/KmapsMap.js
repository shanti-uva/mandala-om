...
class KmapsMap extends React.Component {
  ...
  componentDidMount() {
    ...
    const geoserverUrl = 'http://dev-places.kmaps.virginia.edu:8080/geoserver/';
    const featureLayer = new TileLayer({
          source: new TileWMSSource({
            url: 'http://dev-places.kmaps.virginia.edu:8080/geoserver/wms',
            params: {
              'LAYERS': 'thl:'+layer_name+'_poly,thl:'+layer_name+'_pt,thl:'+layer_name+'_line' ,
    ...
    map.once('postrender',(event) => {
      ...
      fetch(serverUrl).then(res => res.json()).then((result) => {
        //because we are using WFS V1.1 we need to flip the coordinates
        const bbox = [result.bbox[1],result.bbox[0],result.bbox[3],result.bbox[2]];
        var ext = transformExtent(bbox, getProjection('EPSG:4326'), getProjection('EPSG:900913'));
        map.getView().fit(ext, {size: [this.state.height,this.state.width], padding: [1,1,1,1], constraintResolution: false});
      });
    });
  }
  ...
  render() {
    ...
    return (
      //<LoadScript
      //googleMapsApiKey="AIzaSyAXpnXkPS39-Bo5ovHQWvyIk6eMgcvc1q4"
    //>
      <div>
        <div tabindex="1" onClick={this.handleClick} style={style} ref="inset_map"></div>
      </div>
      //</LoadScript>
    );
  }
